...
print('The mean is '+'%.2f' %(sum(L)/len(L))+'.')
L.sort()
if ((len(L)%2)!=0):
	print('The median is '+'%.2f'%(L[int(len(L)/2)])+'.')
else:
	print('The median is '+'%.2f'%((L[int(len(L)/2)]+L[int(len(L)/2)-1])/2)+'.')
a=0
for item in L:
	a+=(item-(sum(L)/len(L))) ** 2
print('The standard deviation is '+'%.2f'% sqrt(a/len(L))+'.')
print()
